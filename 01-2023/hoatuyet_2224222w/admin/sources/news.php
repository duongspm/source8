...
function saveMan()
{
    ...
    if ($id && $act != 'save_copy') {
        ...
        if ($d->update('news', $data)) {
            ...
            if ($func->hasFile("file_tailieu")) {
				$fileUpdate = array();
				$file_name = $func->uploadName($_FILES['file_tailieu']["name"]);

				if ($file_tailieu = $func->uploadImage("file_tailieu", $config['news'][$type]['file_type'], UPLOAD_FILE, $file_name)) {
					$fileUpdate['file_tailieu'] = $file_tailieu;
                    $d->where('id', $id);
            ...
            if (isset($config['news'][$type]['schema']) && $config['news'][$type]['schema'] == true) {
                if ($buildSchema) {
                    foreach ($config['website']['seo'] as $k => $v) {
                        ...
                        $logo = $d->rawQueryOne("select photo from #_photo where type = ? and act = ? limit 0,1", array('logo', 'photo_static'));
                        $dataSchema['schema' . $k] = $func->buildSchemaArticle($id_insert, $data['name' . $k], $configBase . UPLOAD_NEWS_L . @$photo, $data['date_created'], $data['date_created'], $setting['name' . $k], $url_news, $configBase . UPLOAD_PHOTO_L . @$logo['photo'], $configBase);
                    }
                } else {
                    $dataSchema = (isset($_POST['dataSchema'])) ? $_POST['dataSchema'] : null;
                    if ($dataSchema) {
    ...
    if ($id) {
        ...
        if ($d->update('news_list', $data)) {
            ...
            if ($func->hasFile("file_tailieu")) {
                ...
                if ($file_tailieu = $func->uploadFile("file_tailieu", $config['news'][$type]['file_type'], UPLOAD_FILE, $file_name)) {
                    $row = $d->rawQueryOne("select id, file_tailieu from #_news where type = ? limit 0,1", array($type));

                    if (!empty($row)) {
                        $func->deleteFile(UPLOAD_FILE . $row['file_tailieu']);
                    }
